...
class Kategoribidang extends BaseController {
    ...
    public function index(){
        if(session()->get("logged_busdev")){
            ...
            echo view('back/busdev/bidang/kategoribidang');
            echo view('back/foot');
        }else{
    ...
    public function ajaxlist() {
        if(session()->get("logged_busdev")){
            ...
            $list = $this->model->getAllQ("select * from bidangkategori where idbidang = '".$cek."';");
            foreach ($list->getResult() as $row) {
                $val = array();
                $val[] = $no;
                $val[] = $row->namakatbid;
                $val[] = '<div style="text-align: center;">'
                . '<button type="button" class="btn btn-sm btn-warning btn-fw" onclick="ganti('."'".$row->idkatbid."'".')"><i class="fas fa-pencil-alt"></i></button>&nbsp;'
                . '<button type="button" class="btn btn-sm btn-danger btn-fw" onclick="hapus('."'".$row->idkatbid."'".','."'".$row->namakatbid."'".')"><i class="fas fa-trash-alt"></i></button>'
                . '</div>';
                $data[] = $val;
