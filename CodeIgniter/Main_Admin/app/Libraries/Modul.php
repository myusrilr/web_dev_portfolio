...
class Modul
{
    ...
    public function resetAI($nama_tb)
    {
        $stringreset = "ALTER TABLE " . $nama_tb . " AUTO_INCREMENT = 1;";
        return $stringreset;
    }
    ...
    public function KurangTanggal($tgl, $kurang)
    {
        return date('Y-m-d', strtotime('-' . $kurang . ' days', strtotime($tgl)));
    }

    public function convertBulanAngka($bulanString)
    {
        $bulan = "";
        if (strtolower($bulanString) == "januari") {
            $bulan = "01";
        } else if (strtolower($bulanString) == "februari") {
            $bulan = "02";
        } else if (strtolower($bulanString) == "maret") {
            $bulan = "03";
        } else if (strtolower($bulanString) == "april") {
            $bulan = "04";
        } else if (strtolower($bulanString) == "mei") {
            $bulan = "05";
        } else if (strtolower($bulanString) == "juni") {
            $bulan = "06";
        } else if (strtolower($bulanString) == "juli") {
            $bulan = "07";
        } else if (strtolower($bulanString) == "agustus") {
            $bulan = "08";
        } else if (strtolower($bulanString) == "september") {
            $bulan = "09";
        } else if (strtolower($bulanString) == "oktober") {
            $bulan = "10";
        } else if (strtolower($bulanString) == "november") {
            $bulan = "11";
        } else if (strtolower($bulanString) == "desember") {
            $bulan = "12";
        }
        return $bulan;
    }

    public function AkhirTanggal($tgl)
    {
        $tgl_end = date("t", strtotime($tgl));
        return $tgl_end;
    }
    ...
    public function selisih_hari($tgl_awal, $tgl_akhir)
    {
        $d1 = new \DateTime($tgl_awal);
        $d2 = new \DateTime($tgl_akhir);
        $interval = $d2->diff($d1);
    ...
    public function enkrip_random_id($string)
    {
        $secret_key = "83248273547234";
        $secret_iv = "2456378494765431";
        $encrypt_method = "aes-256-cbc";
        // hash
        $key = hash("sha256", $secret_key);
    ...
    public function dekrip_random_id($string)
    {
        $secret_key = "83248273547234";
        $secret_iv = "2456378494765431";
        $encrypt_method = "aes-256-cbc";
    ...
    public function info_file($file)
    {
        ...
        $data['size'] = $file->getSize(); // Ukuran dalam byte
        $data['size_kb'] = $file->getSizeByUnit('kb'); // Ukuran dalam kilobyte
        $data['size_mb'] = $file->getSizeByUnit('mb'); // Ukuran dalam megabyte
    ...
    public function deviceid1()
    {
        return "f03d68f7e9a36d504d368b5dd9c9b1e1";
    }

    public function deviceid2()
    {
        return "bd29916d335f1938eb0b75f0a482816a";
    }
}
