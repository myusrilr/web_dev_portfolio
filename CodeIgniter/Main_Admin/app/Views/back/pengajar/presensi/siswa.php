...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        // console.log("<?php echo base_url(); ?>presensisiswa/ajaxlistsiswa/<?php //echo $head->idjadwal.'/'.$subhead->idjadwaldetil; ?>");
        tb = $('#tb').DataTable({
            ajax : "<?php echo base_url(); ?>presensisiswa/ajaxlistsiswa/<?php echo $head->idjadwal.'/'.$subhead->idjadwaldetil; ?>",
            ordering : false,
            paging : false,
            searching : false
        });
    ...
    function absen(idsiswa, idjadwal){
        var nilai = "tidak";
        ...
        $.ajax({
            url: "<?php echo base_url(); ?>presensisiswa/prosespresensi",
            dataType: 'JSON',
            cache: false,
            contentType: false,
            processData: false,
                ...
                <div class="col-md-5">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <br>
                            <label class="form-label">Tag Materi Diskusi (Jika diperlukan)</label>
                            <select class="select2-demo form-control" multiple style="width: 100%;  z-index:100000;" name="tag[]">
                                <?php foreach($tags->getResult() as $row){ ?>
                                <option value="<?php echo $row->idtagmd; ?>" <?php if($isitag != '') { foreach($isitag->getResult() as $rows) { if($row->idtagmd == $rows->idtagmd){ echo "selected"; } }}?>><?php echo $row->tag; ?></option>
                                <?php } ?>
                            </select>
                        </div>
