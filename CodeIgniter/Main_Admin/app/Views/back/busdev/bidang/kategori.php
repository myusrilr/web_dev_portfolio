<script type="text/javascript">
    ...
    $(document).ready(function () {
        table = $('#tb').DataTable({
            ajax: "<?php echo base_url(); ?>bidangkategori/ajaxlist"
        });
    ...
    function save() {
        ...
        if(tot === 2){
            ...
            var url = "";
            if (save_method === 'add') {
                url = "<?php echo base_url(); ?>bidangkategori/ajax_add";
            } else {
                url = "<?php echo base_url(); ?>bidangkategori/ajax_edit";
            }

            var form_data = new FormData();
            form_data.append('kode', kode);
            form_data.append('judul', judul);
            form_data.append('idbidang', jenis);
    ...
    function hapus(id, nama) {
        swal({
            ...
            function(isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        url: "<?php echo base_url(); ?>bidangkategori/hapus/" + id,
                        type: "POST",
                        dataType: "JSON",
                        error: function() {
                            alert('Error');
    ...
    function ganti(id) {
        ...
        $.ajax({
            url: "<?php echo base_url(); ?>bidangkategori/ganti/" + id,
            type: "POST",
            dataType: "JSON",
            success: function (data) {
                $('[name="kode"]').val(data.idbidangkategori);
                $('[name="jenis"]').val(data.idbidang);
