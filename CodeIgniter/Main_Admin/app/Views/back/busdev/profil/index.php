<script type="text/javascript">
    ...
    $(document).ready(function () {
        table = $('#tb').DataTable({
            ajax: "<?php echo base_url(); ?>leapprofil/ajaxlistt"
        });
    ...
    function save() {
        ...
        if(tot === 2){
            ...
            var url = "";
            if (save_method === 'add') {
                url = "<?php echo base_url(); ?>leapprofil/ajax_add";
            } else {
                url = "<?php echo base_url(); ?>leapprofil/ajax_edit";
            }
    ...
    function hapus(id, nama) {
        swal({
            ...
            function(isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        url: "<?php echo base_url(); ?>leapprofil/hapus/" + id,
                        type: "POST",
                        dataType: "JSON",
                        error: function() {
                            alert('Error');
    ...
    function ganti(id) {
        ...
        $('#form')[0].reset();
        $('#modal_form').modal('show');
        $('.modal-title').text('Ganti Link');
        $.ajax({
            url: "<?php echo base_url(); ?>leapprofil/ganti/" + id,
            type: "POST",
            dataType: "JSON",
            success: function (data) {
                $('[name="kode"]').val(data.idleap);
...
<div class="container-fluid flex-grow-1 container-p-y">
    <h4 class="font-weight-bold py-3 mb-0">Link File Profil Leap</h4>
    <div class="text-muted small mt-0 mb-4 d-block breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><i class="feather icon-home"></i></li>
            <li class="breadcrumb-item active">Link File Profil Leap</li>
        </ol>
    </div>
    <div class="row">
